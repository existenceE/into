...
class BusinessUnit(models.Model):
    ...
    contact = models.ForeignKey('UserGroup', verbose_name='业务联系人', related_name='c',on_delete=models.CASCADE) #组代表多个人
...
class Asset(models.Model):
    ...
    #自定义。文章 服务器 标签 干什么用的 以前历史 web存储缓存 以后方便找到 就像博客标签
    tag = models.ManyToManyField('Tag')

    #最后修改时间。ssh paramiko 获取今天未采集资产。
...
class Server(models.Model):
    ...
    服务器信息
    """
    asset = models.OneToOneField('Asset',on_delete=models.CASCADE)  #归属
    ...
    #每一台服务器出厂都有一个管理IP。服务器没有装系统，内置小系统。可以配置，连上它。执行命令。
